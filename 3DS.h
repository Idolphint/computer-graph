...
#define OBJ_MESH          0x4100                // 新的网格对象,转换矩阵
#define MAX_TEXTURES  10                        // 最大的纹理数目
//  OBJ_MESH的次级定义
...
{public:        float x, y, z;
				...
				CVector3 rotate_(float offset[9]);
				void print();
};
...
public:
        ...
        static CVector3 ComputeNormals(CVector3 p1, CVector3 p2, CVector3 p3);
        float max_length;
        float get_max_length();

private:
        bool Import3DS(t3DModel *pModel, const char *strFileName);// 装入3ds文件到模型结构中
        void CreateTexture(int textureArray[], char* strFileName, int textureID);//  从文件中创建纹理
...
extern int g_Texture[10][MAX_TEXTURES];
extern t3DModel g_3DModel[2]; //gloabal models
#endif
